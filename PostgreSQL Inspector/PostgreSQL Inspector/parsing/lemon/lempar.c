  ...
  do{
    ...
    if( yyact<YYNSTATE ){
      ...
      yymajor = YYNOCODE;
        ...
        if (yymajor == T_WRENCH && !context->parser_state->has_parsed_wrench) {
            context->parser_state->has_parsed_wrench = true;
#ifndef NDEBUG
            if (yyTraceFILE) {
                fprintf(yyTraceFILE, "%sReducing wrench\n", yyTracePrompt);
            }
#endif
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                if (a < YYNSTATE + YYNRULE)
                {
                    ...
                    // printf("wrench: possible token: %s\n", yyTokenName[i]);
                }
            }
        }
